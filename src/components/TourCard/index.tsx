...
const TourCard = ({ imgUrl, locale, title, stars, reviews, days, minPrice }: TourCardProps) => {
  return (
    <a 
      ...
      <button className='absolute bg-white rounded-full p-2 top-4 right-4 hover:text-red-600'>
          <FaRegHeart size={12} />
      </button>
      ...
      <div className='w-[95%] px-2 flex flex-col justify-center gap-1'>
        ...
        <p className='flex justify-between text-gray-500 text-body border-t-2 py-2'>
          Starting from 
          <span className='text-black text-h6 font-price italic font-semibold'>${minPrice}</span>
        </p>
      </div>
    </a>
  );
